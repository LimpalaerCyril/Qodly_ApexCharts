...
const Area: FC<IAreaProps> = ({ xAxisTitle, yAxisTitle, strokeCurve, chartType, exportable, zoomable, titlePosition, legendPosition, name, style, className, classNames = [] }) => {
  const {
    connectors: { connect },
  } = useEnhancedNode();
  ...
  const options: ApexOptions = useMemo( // Prevents unnecessary re-renders if no editor changes
    () => ({
      chart: {
        type: chartType,
        zoom: {
